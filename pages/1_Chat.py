...
def ask_model(url_server: str, route_status:str, route_model: str, llm_question: LLMQuestion) -> LLMResponse:
    if server_is_online(url_server, route_status):
        response = httpx.post(f'{url_server}{route_model}', json=llm_question.model_dump())
        if response.status_code == 200:
...
if question := st.chat_input("input..."):
    ...
    with st.chat_message("assistant"):
        ...
        llm_result = ask_model(url_server, configuration.server.routes.status, configuration.server.routes.model, llm_question)
